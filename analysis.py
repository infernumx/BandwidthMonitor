...
filename = max(glob.glob('dumps/*'), key=os.path.getctime)
data_by_hour = [[0, 0]]
...
def process_dumpfile(filename):
    ...
    with open(filename) as f:
        for line in f:
            ...
            delta = datetime.timedelta(seconds=network_data.time.second,
...
def create_line_graph(data, y_axis):
    df = pd.DataFrame({
        y_axis: data,
        'Hours': [i for i in range(len(data))]
    })
...
data = [{'Sent': sent,
        'Received': received}
        for sent, received in data_by_hour]

data_length = len(data_by_hour)
fill = len(str(data_length))

df = pd.DataFrame(data, index=[f'Hour {str(i).zfill(fill)}' for i in range(data_length)])
st.line_chart(df)
